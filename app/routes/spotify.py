...
@router.post('/api/create-playlist', name='spotify.api_create_playlist',
             dependencies=[Depends(rate_limit(max_requests=10, window_seconds=60))])
async def api_create_playlist(request: Request):
    """API endpoint: Create a private playlist from top tracks."""
    require_oauth(request)
    body = await _get_json_body(request)

    time_range = body.get('time_range', 'short_term')
    if time_range not in ('short_term', 'medium_term', 'long_term'):
        time_range = 'short_term'

    size = body.get('size', 20)
    if not isinstance(size, int) or size not in (10, 20, 30):
        size = 20

    # Get user profile for user_id
    user_data, user_status = await spotify_request(request, '/me')
    if not user_data or 'id' not in user_data:
        raise HTTPException(status_code=user_status or 500, detail='Failed to fetch user profile')
    ...
    if not tracks_data or 'items' not in tracks_data or not tracks_data['items']:
        raise HTTPException(status_code=tracks_status or 500, detail='No top tracks found')

    track_uris = [track['uri'] for track in tracks_data['items']]

    # Create private playlist
    range_labels = {'short_term': '4 Weeks', 'medium_term': '6 Months', 'long_term': 'All Time'}
    playlist_name = f"Top {len(track_uris)} - {range_labels[time_range]}"

    playlist_data, playlist_status = await spotify_request(
        request, f'/users/{user_id}/playlists', method='POST',
        json_body={'name': playlist_name, 'public': False, 'description': 'Generated by alexmatthew.me'}
    )

    if not playlist_data or 'id' not in playlist_data:
        if playlist_status == 403:
            raise HTTPException(status_code=403, detail='Missing playlist scope - please reconnect')
        raise HTTPException(status_code=playlist_status or 500, detail='Failed to create playlist')

    playlist_id = playlist_data['id']

    # Add tracks
    _, add_status = await spotify_request(
        request, f'/playlists/{playlist_id}/tracks', method='POST',
        json_body={'uris': track_uris}
    )

    if add_status not in (200, 201):
        raise HTTPException(status_code=add_status or 500, detail='Failed to add tracks to playlist')
